...
> **Amended after code review.** The CSS block below is the original draft.
> Code quality review found the variant names mixed two axes — `--primary` was
> the white button and `--dark` the navy one, which are not opposites — so a
> reader building on the light Who-are-we ground would reach for "primary" and
> get a white pill on white. They were renamed `--on-dark` / `--on-light`
> before Task 3 wrote them into markup. The shine was also taken off the navy
> variant. **The committed `assets/css/sections.css` is the source of truth,
> not this block.**
